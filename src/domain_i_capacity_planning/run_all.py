"""Run capacity planning benchmark with reproducible multi-seed evaluation."""
...
from .data_generator import create_capacity_planning_dataset
...
def _capacity_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
...
def run_all_approaches(
    ...
    output_dir: str = "results/domain_i",
    n_runs: int = 1,
    seed: int = 42,
    ...
    print("=" * 70)
    print("CAPACITY PLANNING: 10 Approaches Comparison")
    print("=" * 70)
    ...
    for run_idx, run_seed in enumerate(seeds, start=1):
        ...
        dataset = create_capacity_planning_dataset(n_samples=n_samples, n_features=n_features, seed=run_seed)
        ...
        for name, category, model_factory, custom_predict in _approach_specs(smoke_test=smoke_test):
            try:
                ...
                metrics = _capacity_metrics(y_test, y_pred)
                ...
                row = {
                    ...
                    "philosophy": {
                        ...
                        "best_for": "Capacity planning simulations",
                    },
                    "metrics": metrics,
