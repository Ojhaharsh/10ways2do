"""Run model risk monitoring benchmark with reproducible multi-seed evaluation."""
...
from .data_generator import create_model_risk_dataset
...
def _model_risk_metrics(y_true: np.ndarray, y_pred: np.ndarray, y_score: np.ndarray) -> Dict[str, float]:
    return {
...
def run_all_approaches(
    ...
    output_dir: str = "results/domain_j",
    n_runs: int = 1,
    seed: int = 42,
    ...
    print("=" * 70)
    print("MODEL RISK MONITORING: 10 Approaches Comparison")
    print("=" * 70)
    ...
    for run_idx, run_seed in enumerate(seeds, start=1):
        ...
        dataset = create_model_risk_dataset(n_samples=n_samples, n_features=n_features, seed=run_seed)
        ...
        for name, category, model_factory, custom_eval in _approach_specs(smoke_test=smoke_test):
            try:
                ...
                metrics = _model_risk_metrics(y_test, y_pred, y_score)
                ...
                row = {
                    ...
                    "philosophy": {
                        ...
                        "best_for": "Model governance and production risk triage",
                    },
                    "metrics": metrics,
