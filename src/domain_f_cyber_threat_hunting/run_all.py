"""Run cyber threat hunting benchmark with reproducible multi-seed evaluation."""
...
from .data_generator import create_threat_hunting_dataset
...
def _threat_metrics(y_true: np.ndarray, y_pred: np.ndarray, y_score: np.ndarray) -> Dict[str, float]:
    return {
    ...
    specs = [
        (
            ...
            lambda model, x_train, y_train, x_test: (
                (x_test[:, 0] + x_test[:, 1] > np.quantile(x_train[:, 0] + x_train[:, 1], 0.88)).astype(int),
        ...
        (
            ...
            "classical_ml",
            lambda seed: make_pipeline(StandardScaler(), LogisticRegression(max_iter=300, random_state=seed)),
            None,
        ),
        (
...
def run_all_approaches(
    ...
    output_dir: str = "results/domain_f",
    n_runs: int = 1,
    seed: int = 42,
    ...
    print("=" * 70)
    print("CYBER THREAT HUNTING: 10 Approaches Comparison")
    print("=" * 70)
    ...
    for run_idx, run_seed in enumerate(seeds, start=1):
        ...
        dataset = create_threat_hunting_dataset(n_samples=n_samples, n_features=n_features, seed=run_seed)
        ...
        for name, category, model_factory, custom_eval in _approach_specs(smoke_test=smoke_test):
            try:
                ...
                metrics = _threat_metrics(y_test, y_pred, y_score)
                ...
                row = {
                    ...
                    "philosophy": {
                        ...
                        "best_for": "Threat prioritization prototyping",
                    },
                    "metrics": metrics,
